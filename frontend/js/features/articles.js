...
function renderArticleCard(a, issue = null, relatedMembers = []) {
  ...
  if (evidenceFailure) badges.unshift(`<span class="badge badge-evidence-error" title="${escapeAttr(evidenceFailureReason(evidenceFailure))}">MD 전체 차단</span>`);
  const domainLabel = {
    electrical: "전기적 요인",
    battery: "배터리 요인",
    mechanical: "기계적 요인",
...
function renderRelatedArticle(a, issue) {
  ...
  return `<li class="related-article-row related-role-${escapeAttr(quality.role || "related")}" data-id="${escapeAttr(a.id)}" data-issue-id="${escapeAttr(issue.id)}">
    ...
    <div class="related-article-content">
      ...
      <div class="related-quality-badges">${evidenceFailure ? `<span class="badge badge-evidence-error" title="${escapeAttr(evidenceFailureReason(evidenceFailure))}">MD 전체 차단</span>` : ""}${quality.extractionMethod === "manual_paste" ? '<span class="badge badge-manual">수동 본문</span>' : ""}<span class="evidence-role role-${escapeAttr(quality.role || "related")}">${roleLabel}</span><span class="quality-status status-${escapeAttr(quality.extractionStatus || "not_attempted")}">${statusLabel}</span><span class="quality-grade grade-${escapeAttr(quality.qualityGrade || "unavailable")}">${gradeLabel} ${Number(quality.contentQualityScore || 0)}</span></div>
