...
- 재그룹화는 proposal/apply 방식이며 담당자 override를 덮어쓰지 않는다.
- JSON은 정식 백업, CSV는 손실형 목록 교환 형식이다.

## Phase 9 운영 명령

최초 설정 후 평소에는 `start_kesco_briefing.command`를 실행한다. 서버는 health의 서비스
식별자와 파일 잠금을 함께 확인해 중복 기동을 막는다.
...
실행 중인 서버는 화면 우측 상단 설정을 열어 `서버 재시작` 버튼으로 바로 재시작할 수 있다.
새 서버의 health가 확인되면 브라우저 화면도 자동으로 새로고침된다.

로그인 시 서버 자동 시작과 2시간 주기 자동수집은 다음 명령으로 설치한다. 설치 전에
화면의 `검색 설정`을 검토한다. 기본값은 `config/collection_settings.json`, 사용자 변경은
SQLite settings override에 저장되어 수동·자동 수집에 함께 적용된다.

```bash
./install_launchd.command install
./install_launchd.command status
./install_launchd.command uninstall
```

DB 수동 백업과 복구는 다음과 같다. 복구할 때는 앱과 launchd 서버를 먼저 중지해야 한다.
...
상세 절차와 1~2주 병행운영 기준은 `docs/OPERATIONS_RUNBOOK.md`와
`docs/PARALLEL_OPERATION_CHECKLIST.md`를 따른다.
