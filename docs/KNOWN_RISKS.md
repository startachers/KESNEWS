...
| LEG-010 | 재그룹화 모델이 아직 없어 향후 수동 이슈 편집 보호 계약이 필요함 | 자동 재분석이 담당자 편집을 덮을 위험 | API·DB 계약 선확정 | **Phase 6에서 해소.** 그룹 실행을 proposal/apply로 분리하고 `issues.editor_*`와 `issue_membership_overrides`를 자동 결과와 별도로 저장한다. apply는 editor 필드와 수동 add/remove를 유지하며 effective 값은 editor/override를 우선한다. `tests/integration/test_clustering_api.py::test_recluster_apply_preserves_editor_fields_and_membership_overrides` |
...
| ID | 후속 필요 사항 | 배경 | 처리 Phase |
|---|---|---|---|
| P3-001 | `POST /api/collections` 요청 바디가 최종 계약(`API_DATA_CONTRACTS.md` §3.5, `{report_date, lookback_hours}`만 받음)과 달리 검색식·키워드·lookback 등 현재 localStorage 설정 전체와 `existingArticles`를 그대로 실어 보낸다 | 설정이 아직 서버에 없다(Phase 4 `/api/settings` 대상). Phase 3 범위를 "localStorage 유지 가능"으로 한정한 REFACTORING_MAP §5에 따른 임시 절충 | **2026-07-21 해소.** `existingArticles`는 먼저 제거했고, P4-001 완료로 검색식·키워드·provider 설정도 서버 `/api/settings`로 옮겼다. 현재 요청 바디는 `report_date`와 `lookback_hours`만 사용하며 구버전 추가 필드는 무시한다. |
...
| ID | 후속 필요 사항 | 배경 | 처리 Phase |
|---|---|---|---|
| P4-001 | 검색식·키워드가 수집 요청 바디, localStorage, 자동수집 JSON에 중복됐다 | 수동·자동 수집 설정 불일치와 오래된 브라우저 설정 위험 | **2026-07-21 해소.** `config/collection_settings.json` 기본값과 `settings.key=collection` 사용자 override를 `GET/PUT/reset /api/settings`로 통합했다. 수동·자동 수집 요청은 `report_date`와 `lookback_hours`만 보내고 구버전 설정 필드는 무시한다. 기존 localStorage 검색 설정은 override가 없을 때 1회 이전한다. `tests/integration/test_settings_api.py` |
...
| P4-006 | CSV export/import의 분류 컬럼이 내부 category ID를 그대로 표시했다 | 설정과 CSV가 분리돼 한글 라벨을 안정적으로 역변환할 수 없었다 | **P4-001과 함께 해소.** 유효 서버 설정의 검색 그룹 ID↔한글 label 매핑으로 CSV를 내보내고 가져올 때 내부 ID로 복원한다. 설정에 없는 과거 category는 원시값을 보존한다. `tests/integration/test_exports.py::test_csv_round_trip_preserves_incident_and_exports_unknowns_as_blanks` |
...
| NC-004 | 네이버 뉴스 API provider와 실행 시 `.env` 로딩이 없었다 | Google RSS에만 잡히지 않는 허용 언론사 기사를 놓치고, 루트 `.env`의 자격정보를 실행 코드가 읽지 못했다. | **단계 4에서 해소.** 신규 의존성 없는 `KEY=value` 로더를 서버·자동수집 시작 시 적용하고, 현재 21개 검색군의 `naverQueries`를 최대 3개씩 네이버 공식 API에 전달한다. `originallink` 기준 허용목록 판별, 최대 3페이지·lookback 중단, Google RSS 병행, 네이버 장애 warning 폴백, 화면 3상태, 자격정보 미노출을 구현했다. 같은 기사의 provider별 observation도 모두 보존한다. `tests/unit/test_naver_news.py`, `tests/integration/test_collection_pipeline.py` |
...
| NC-006 | 수집한 정부부처 원문(NC-005)이 CEO 브리핑에서 별도 섹션 없이 ④ 기타 동향에 섞이거나, OUT_OF_SCOPE 판정으로 아예 누락됐다 | 정부부처 성격이 선정 단계 정규식 태그로만 존재하고 분석·렌더까지 전달되지 않아, 정부 정책 동향을 CEO가 한눈에 보기 어려웠다 | **2026-07-23 구현.** `is_government_article`(출처 `governmentPressRelease` 또는 정부 메시지 카테고리)로 판정을 결정론화하고 선정→분석(`governmentSource`)→렌더(`_render_government_reference`)로 전달한다. ④ 기타 동향을 없애 비정부 동향은 ③ 경영 참고사항에 병합하고, 정부부처 기사는 ④ 정부부처 동향으로 분리한다. 정부 기사는 OUT_OF_SCOPE여도 reference keyIssue로 보존하도록 프롬프트를 강화했다. **남은 위험:** (1) 정부부처 최소 2건은 프롬프트 선호일 뿐 하드 검증이 없어 담당자가 정부부처 필터에서 직접 체크하지 않고 정부 후보도 부족한 날은 ④가 비어 렌더되지 않는다. (2) 섹션 분할은 스냅샷 `evidence[*].article`의 `governmentPressRelease`/`category`에 의존하므로, `_evidence_snapshot` 폴백(기사가 `articles_by_id`에 없을 때)처럼 두 필드가 없는 경로에서는 정부 기사가 비정부로 취급될 수 있다. `tests/unit/test_government_briefing_section.py` |

검색 설정 이중 원본은 P4-001에서 해소했다. provider 자격정보는 설정 API에 넣지 않고
기존 원칙대로 서버 환경변수에서만 읽는다.
