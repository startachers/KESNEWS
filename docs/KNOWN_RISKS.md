...
| ID | 후속 필요 사항 | 배경 | 처리 Phase |
|---|---|---|---|
| P4-001 | 검색식·키워드(`queries`/`coreKeywords`/`riskKeywords`/`positiveKeywords`/`excludeKeywords`/`endpoint`)가 여전히 `POST /api/collections` 요청 바디로 전달되고 `settings` 테이블은 스키마만 있고 읽고 쓰는 코드가 없다 | `/api/settings` API 도입 자체가 Phase 4 체크포인트 3 지시서에서 범위 밖으로 명시됨. 기사 수집 설계 변경 단계 1에서 17개 검색식이 localStorage 기본값과 `config/automated_collection.json`에 함께 복제돼 수동·자동 수집의 이중 설정 위험이 더 명확해졌다. | `/api/settings` 도입 Phase에서 `new_rules_news_clip.md` §3 검색식을 단일 원본으로 이관하고 요청 바디를 최종 계약(API_DATA_CONTRACTS.md §3.5)대로 축소 |
