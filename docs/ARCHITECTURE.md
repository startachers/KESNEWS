...
추천 순위순으로 비어 있는 Top Issues 자리만 최대 6개까지 채운다. 추천 기사가 적용된 군집에
속하면 군집의 `briefing_issues.selected`를 활성화하고, 군집이 없는 단독 기사만
`briefing_articles.top_issue`를 활성화한다.
...
AI 분석은 단일 Mac의 GPU 과점유를 막기 위해 앱 전체에서 한 번에 하나만 실행한다. 프런트의 취소 버튼, 브라우저 연결 종료, 경영메시지 10분 총 제한은 모두 같은 취소 token으로 Ollama 스트리밍 연결을 닫는다. 기사 추천은 기존 5분 제한을 유지한다. 앱 시작 시 `running`으로 남은 고아 실행은 `AI_INTERRUPTED` 실패로 복구한다.
...
요약과 경영 영향) 순서로 조립한다. 참고 동향은 공사의 즉시 안전대응 핵심과 분리할 내부
경영관리·제도 흐름이며, 근거가 없을 때 모델이 억지로 생성하지 않는다. 나머지 구조화 필드는
