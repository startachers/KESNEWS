...
압축한 뒤 Gemma가 내용·태그·경영 영향을 재평가해 최대 20건을 추천한다. 최종 출처 판정이
`kesco_republication` 또는 `kesco_based`인 공사 보도자료 전재·기반 기사는 압축 전 후보에서
제외한다. 개별 지자체·소방서의 저심각도 예방 홍보처럼 지역에만 국한된 일상 활동도 제외하되,
실제 사고·인명피해·광역 영향이 있는 지역 기사는 유지한다. 해외 사고는 제거하지 않고 후보
점수를 감점하며, 국내 파급이나 공사 직접성이 명확하면 Gemma가 추천할 수 있게 한다. 후보가
있는 경우 정부부처·경제·AI 분야를 각각 최소 1건 확보하고 서버가 Gemma 결과의 분야 충족을
검증한다. 추천 실행과
`briefing_articles.selected` mutation은 분리하며 담당자의 명시적 적용 전에는 작업본을
변경하지 않는다. 적용 시에도 기존 수동 선정과 메모·중요·Top Issue·분류 override를 보존한다.
...
아니며 담당자가 군집 또는 개별 기사에 지정한 태그를 합쳐 최대 5개 표시한다. 군집 태그는
