...
품질 점수는 관련기사 비교·정렬용이며, 기본 적격성 조건을 통과한 정제 본문을 점수만으로
차단하지 않는다. 성공적으로 제거된 페이지 부가 콘텐츠와 언론사 AI 부가 섹션은 추적 사유로
남기되 감점하거나 정제 본문의 분석 적격성을 해제하지 않는다.
본문 끝의 잔여 페이지 조각은 완결 문장이 2개 이상 확보된 경우 품질 표시에만 반영하고
대표기사 자동 선정과 AI 근거 포함을 차단하지 않는다.
