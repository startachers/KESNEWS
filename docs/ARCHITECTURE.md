...
정부부처 직접 수집 자료는 발표 기관별 독립 원문으로 취급해 자동 동일 이슈 그룹화에서 제외한다.
수집 실행 순서와 무관하게 기존 기사와 observation은 누적 보존하며, 담당자의 수동 그룹만 예외로
허용한다.
...
브리핑 기사 추천도 같은 원칙을 따른다. 기존 분류·그룹 검토점수로 후보를 최대 60건까지
...
담당자가 Media Coverage에서 수동으로 관리하며, 추천 적용은 그룹의
`briefing_issues.selected`나 개별 기사의 `briefing_articles.top_issue`를 변경하지 않는다.
기존 버전에서 그룹 기사에 직접 저장된 Top 태그는 migration 0016에서 그룹 태그로 승격한다.
화면과 상한 계산도 그룹 구성 기사의 잔여 태그를 해당 그룹 하나로 합산해, 비활성 버튼과
...
동일 사건·주제를 다룬 기사 묶음이다. 자동 그룹 결과와 담당자 편집값을 분리한다.
...
기사 구성은 자동 membership과 담당자 add/remove override를 분리한다. 재그룹화가 담당자 제목·상태·구성을 덮어쓰지 않는 상세 규칙은 `docs/API_DATA_CONTRACTS.md` 6장을 따른다.
...
article_body_overrides
  article_id                                기사별 수동 본문 1건
  extraction_id                             검증된 manual_paste 이력
  raw_text, cleaned_text, source_url
...
이슈에 수동 membership으로 연결하므로 이후 자동 재그룹화가 담당자의 연결을 덮어쓰지 않는다.
...
그룹 검토순위는 동일 이슈라도 보고일 후보 집합에 따라 달라지므로 `issues`에 저장하지 않는다.
...
- 기사 또는 그룹 Top 태그 활성화: 해당 기사 `selected=true`; Top 해제 시 선정은 유지
...
그룹 자동값은 `issues.direct_mention`이다. 보고일별 담당자 override는 각각
`briefing_articles.direct_coverage_override`, `briefing_issues.direct_coverage_override`에
저장하며 단독 기사 override는 최초 그룹화 뒤에도 상속한다. 유효값이 참인 기사·그룹은 일반 브리핑과
...
- 외부 AI 일반 텍스트의 `오늘 한줄`, `언론 동향 분석`, `경영 참고사항`, `기타 동향` 제목과
...
재그룹화는 proposal을 먼저 만들고 apply 단계에서 자동 필드만 갱신한다. 담당자 `editor_*` 값과 수동 add/remove는 유지한다.
...
화면의 Media Coverage는 적용된 이슈 그룹으로 기사를 묶어 표시한다. Top Issues는 자동 순위가
아니며 담당자가 그룹 또는 개별 기사에 지정한 태그와 명시적으로 적용한 Gemma 핵심 추천을
합쳐 최대 6개 표시한다. 그룹 태그는
...
그룹과 개별 기사에 저장된 `sort_order`를 하나의 카드 배치 순서로 사용한다.
기사별 별도 `그룹 선택` 체크박스로 2건 이상을 선택하면 수동 이슈를 생성할 수 있다. 수동
그룹은 `issues.manual_group`과 `issue_membership_overrides`의 add/remove로 저장하며 자동
재그룹화가 선택 기사를 다른 이슈에 중복 포함하지 않도록 적용 후 배타성을 복원한다.
...
정책브리핑 보도자료 OpenAPI는 `POLICY_BRIEFING_SERVICE_KEY`가 설정된 경우에만 수집한다.
보고일 기준 전일~당일을 `startDate`·`endDate`로 조회하고 XML 원문 필드를 정규화해 일반
`Article`/`ArticleObservation` 파이프라인에 넣는다. 정부부처 observation 존재 여부는
`governmentPressRelease` 표시값으로 조회되며 Media Coverage의 자료 유형 필터가 사용한다.
공식 정부부처 수집 자료는 관련도·전체 제외어로 제거하지 않되 자동 후보 24시간 제한은 적용한다.
발행시각이 없는 국무조정실·기후에너지환경부 목록은 서울 기준 보고일·전일의 날짜 범위로 24시간
후보를 근사하고 2일 전 자료부터 제외한다. 날짜를 UTC 자정으로 간주해 전일 자료가 오전 9시에
일괄 만료되지 않도록 수집 단계와 후보 조회 단계에서 같은 규칙을 사용한다.
같은 provider 안에서는 동일 `sourceId`만 중복으로 병합해 수집된 개별 보도자료를 보존한다.
필터는 화면 표시만 바꾸므로 브리핑 선정 이후 AI 분석·CEO 보고 snapshot 흐름은 언론기사와 같다.
`POST /api/collections`는 기존 언론기사·국무조정실·기후에너지환경부 직접 수집을 유지하되
정책브리핑 API는 실행하지 않는다. 별도 `POST /api/government-press-releases/collections`는
공공데이터포털 정책브리핑 API만 실행하며 실패 시 직접 수집이나 언론기사로 대체하지 않는다.
정책브리핑 API의 정상 응답 항목은 모두 `governmentPressRelease` 표시값을 갖고 동일 저장
파이프라인에 들어가므로 정부부처 필터, 브리핑 선정, CEO 보고에서 그대로 사용할 수 있다.

### 10.4 Fuzzy duplicate detection

완전 중복과 동일 이슈 그룹화를 구분한다.
...
일반 기사 수집은 외부 공사 사이트를 다시 호출하지 않고 저장 원문만 읽는다. 기사 정규화·분류 직후, 그룹화 전에
...
심각도 점수: severity_score 0~100(그룹 검토점수의 내부 근거)
그룹 검토점수: review_score 0~100
그룹 검토별점: 1~5, 보고일별 상위 10·20·40·100위와 점수 하한 병용
...
신규 화면·정렬·최종 보고는 그룹 검토별점을 사용한다. 관련도 30%, 경영영향도 25%,
...
- 같은 공사 보도자료 ID에 연결된 기사 쌍은 출처 원문을 강한 그룹 anchor로 사용한다.

재그룹화는 `cluster_run` proposal과 apply 두 단계로 처리한다. `editor_title`, `editor_status`, `editor_priority`, 수동 add/remove membership은 자동 결과로 덮어쓰지 않는다. 상세 계약은 `docs/API_DATA_CONTRACTS.md` 6장을 따른다.
...
상세 선택·삭제·근거·재그룹화 계약은 `docs/API_DATA_CONTRACTS.md`가 우선한다.
...
재그룹화 첫 요청은 `0.15~0.70` 범위의 선택적 `similarityThreshold`(기본 `0.40`)로 diff proposal을 만들고, apply 시 자동 필드만 갱신한다. 브라우저는 기사량이 많은 작업본도 계산을 기다릴 수 있도록 제안 생성 요청에 120초 제한을 적용한다. 화면에서 기준을 바꾼 경우 새 proposal을 계산하기 전에는 이전 proposal을 적용할 수 없다.

수동 `오늘 기사 검색`과 화면 진입 시의 당일 자동 검색은 수집 성공 후 `0.15` 기준 cluster
proposal을 생성하고 즉시 apply한다. 화면 진행률은 수집 요청, 기사 목록 갱신, proposal 계산,
apply 완료라는 실제 경계에서 갱신한다. 재그룹화 실패는 수집된 기사를 롤백하지 않는다.
...
근거가 없는 기타 동향도 제외한다. 통과 항목과 그 기사 ID만 두 번째 최종 작성 호출에 전달한다.
...
`actionItems`) 순서로 조립하고, 모니터링 이슈가 있을 때만 `④ 기타 동향`을 추가한다.
기타 동향은 공사의 즉시 안전대응 핵심과 분리할 정책·산업 흐름이며, 근거가 없을 때 모델이
...
- 미리보기에서 전달한 기사 표시 순서와 AI 본문 요약이 있으면 현재 선정 기사 ID·요약 생성
  revision을 검증하고 해당 최종 snapshot에만 보존한다.
- 미리보기 탭과 편집 탭은 보고일·revision 키의 동일 출처 임시 상태로 위 표시값을 공유한다.
  편집 탭의 최종 확정도 이 값을 함께 보내며 성공 후 임시 상태를 제거한다.
- `briefing_versions`에 version N의 불변 JSON snapshot을 추가한다.
- `reports/YYYY/MM/`에 같은 version의 읽기 전용 HTML을 저장한다.
- 작업본 상태를 `final`로 잠근다.
- DB 백업을 실행한다.

수정이 필요하면 메인 화면의 `확정 취소`가 직전 최종 snapshot을 읽고 `reopen`으로 작업본을
다시 연 뒤, 기사 표시 순서와 AI 본문 요약을 새 작업본 revision의 브라우저 미리보기 임시 상태로
복원한다. 기존 version은 보존한다. 재확정 시 N+1 snapshot을 만든다. `/report/{date}`는 최신
최종 version을, `/preview/{date}`는 현재 작업본과 복원된 미리보기 임시 상태를 사용한다.
...
완료 기준: 동일 사건 기사 여러 건을 원본 보존 상태로 볼 수 있고 재그룹화가 담당자 제목·상태·구성을 덮어쓰지 않는다.
...
### ADR-011: 재그룹화는 proposal/apply이며 editor override를 덮지 않는다

자동 그룹 품질을 개선해도 담당자의 제목·상태·구성 편집은 우선한다.
...
- 담당자가 붙여넣은 본문은 `article_body_overrides`에 자동 원문과 분리 저장한다. 조회·품질 판정·
  AI 근거 구성에서는 이 override가 이후 자동 추출 이력보다 우선하며 담당자의 재저장만 이를
  교체한다. 실제 언론사 URL을 함께 입력하면 동일 출처 검증을 다시 수행한다.
