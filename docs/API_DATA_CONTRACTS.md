...
- 최종 선정 12건에는 공사 경영과 직접 연관된 유효 후보가 있는 경우에만 정부부처·경제·AI
  분야를 각각 1건 이상 포함한다.
...
- 동일 이슈는 대표기사 1건을 원칙으로 하며 새로운 사실을 더하는 보조기사까지 최대 2건만
  후보와 추천 결과에 포함한다.
- Gemma는 현재 선정 수가 12건에 도달할 때까지 필요한 수만 추천한다. 1~6위는 핵심 선정,
  7~12위는 추가 참고로 구분한다. 유효 후보가 부족하면
  가능한 수만 추천한다.
