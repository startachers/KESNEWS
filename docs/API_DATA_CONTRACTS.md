...
CEO 작업본 미리보기의 기사 본문 요약은 다음 읽기 전용 AI 요청으로 생성한다.

```text
POST /api/briefings/{date}/article-summaries
```

- 요청은 로컬 Ollama `model`을 받으며 현재 선정 기사의 ID·제목·본문을 입력으로 사용한다.
- 응답은 기사별 `{articleId, summary}`와 생성 기준 `sourceRevision`을 반환한다.
- 요약은 카드 제목 바로 뒤에 이어지는 본문으로 작성하며, 제목이 이미 전달한 주체·사건·결과를
  반복하지 않고 제목에 없는 원인·배경·수치·진행 상황·영향으로 시작한다. 제목 중복 검사를
  통과하지 못한 항목은 한 번 교정하고, 교정 후에도 중복이면 전체 결과를 적용하지 않는다.
- 결과는 먼저 미리보기 DOM과 해당 시점의 인쇄에만 반영하며 기사 원본, 작업본, AI 분석 실행을
  변경하거나 자동 확정하지 않는다. 사용자가 같은 미리보기에서 `finalize`를 명시적으로 실행하면
  클라이언트는 현재 화면의 기사 순서 `articleOrder`, AI 요약 `articleSummaries`, 요약 생성 기준
  `articleSummarySourceRevision`을 함께 보낸다. 서버는 기사 ID 전체 일치·중복·revision을 검증한 뒤
  순서와 `reportSummary`를 해당 최종 snapshot에만 보존한다. 새로고침 전 확정하지 않은 미리보기
  요약은 작업본에 저장하지 않는다. 별도 탭인 편집 화면에서 최종 확정하는 흐름을 위해 이 세 값은
  보고일·revision 키의 동일 출처 브라우저 임시 상태로 공유하며, 확정 성공 시 제거한다.
- 기사 ID 누락·중복 또는 빈 요약은 성공으로 처리하지 않는다. 다른 AI 실행 중이거나 Ollama가
  오프라인이면 기존 기사 본문을 유지하면서 명시적 오류를 반환한다.
...
- 정부부처 직접 수집 observation이 연결된 기사는 자동 재그룹화에서 다른 기사와 병합하지 않고
  각각 독립 카드로 유지한다. 정부자료 수집과 일반 기사 수집의 실행 순서는 기사 원본,
  observation, 선정·중요·메모 상태에 영향을 주지 않는다. 담당자가 명시적으로 만든 수동 그룹과
  membership override는 이 자동 분리 규칙보다 우선해 보존한다.
...
4. `④ 기타 동향`: `MONITORING` 또는 `urgency=reference`인 `keyIssues`가 있을 때만 표시

경영 참고사항이 없으면 `직접적인 경영 현안은 제한적입니다.`로 표시하고, 기타 동향 근거가
...
- ChatGPT·Claude 바로가기는 외부 유료 API를 호출하지 않는다. 사용자가 먼저 Markdown을
  생성한 뒤 바로가기 버튼을 누르면 고정 분석 프롬프트를 클립보드에 복사하고 각 서비스의
  새 대화 화면만 연다. 파일 첨부와 프롬프트 전송은 사용자가 직접 수행한다.
...
- validate는 저장하지 않으며 현재 입력 서명을 검증한다. 일반 텍스트 입력은 현재 선정 기사
  ...
  `경영 참고사항`, `기타 동향`을 내부 구조로 분리한다. 과거 `오늘의 핵심`, `경영 시사점`,
  `언론 동향 시사점`, `경영 참고사항`, `참고 동향` 제목도
