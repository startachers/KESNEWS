...
- 전체 단계와 발표·검토 시각
- 연속된 위험 예보 기간을 `금요일~일요일` 형식으로 묶은 선제대응 문장 최대 3건
- 영향 권역·강수확률·최고기온·전기안전 우려·사전 점검사항
- provider 오류 또는 stale 경고
- 요일별 카드는 노출하지 않고 위험 예보와 대비사항 중심으로 축약
...
기상정보가 보고에서 제외되면 section을 빈 상태로 만들지 않고 생략한다. 다만 `unknown` 또는
수집 오류를 담당자가 보고에 포함하도록 선택한 경우에는 오류 상태도 그대로 인쇄한다.
