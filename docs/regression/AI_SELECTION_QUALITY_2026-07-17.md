...
> 2026-07-20 정책 변경: Gemma 추천 적용의 Top Issues 자동 채움은 제거됐다.
> 현재는 추천 기사 선정만 적용하며 Top Issues는 담당자가 수동 관리한다.
...
- 추천 기사가 그룹에 속하면 기사 태그 대신 해당 그룹의 Top 태그를 활성화
- 구버전의 숨은 기사 Top 태그를 그룹 Top 태그로 승격하는 migration 및 화면 fallback
...
6칸을 추천 중요도 1~6위 순서로 채움, 추천 기사의 그룹 Top 태그 활성화, 그룹이 없는 기사의
...
- 기존 그룹 품질이 낮으면 동일 이슈 대표 1건 제한이 완전하게 작동하지 않을 수 있다.
